...
@if (isset($category_type))
    @section('title', __('settings::dashboard.settings') . ' - ' . __('settings::dashboard.edit_category_type'))

    @section('subheader')
        @include('dashboard.layouts.partials.sub_header', [
            'module_name' => __('settings::dashboard.edit_category_type'),
            'short_description' => __('settings::dashboard.enter_category_type_details_and_submit'),
            'breadcrumbs' => [],
        ]);
    @endsection
@else
    @section('title', __('settings::dashboard.settings') . ' - ' . __('settings::dashboard.create_category_type'))
    @section('subheader')
        @include('dashboard.layouts.partials.sub_header', [
            'module_name' => __('settings::dashboard.new_category_type'),
            'short_description' => __('settings::dashboard.enter_category_type_details_and_submit'),
            'breadcrumbs' => [],
        ]);
    @endsection
    ...
    <div class="card card-custom">
        <div class="card-header">
            <div class="card-title">
                <h3 class="card-label">
                    {{ isset($category_type) ? __('settings::dashboard.edit_category_type') : __('settings::dashboard.new_category_type') }}
                    <i class="mr-2"></i>
                    <small class=""></small>
                </h3>
            </div>
                ...
                <div id="kt_category_type_translation_repeater">
                    <div class="form-group row">
                        <div data-repeater-list="translations" class="col-lg-12">
                            @forelse (old('translations', $category_type->translations ?? []) as $translation)
                                    ...
                                    <div class="col-md-3">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-8">
                                                ...
                                                @endif
                                                <span
                                                    class="form-text text-muted">{{ __('settings::dashboard.the_language_of_the_category_type_name') }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-1">
                        <x-dashboard.form.inputs.success-switch class="mx-2" :id="'is_active'" :name="'is_active'"
                            :isChecked="old('is_active', $category_type->is_active ?? '')" />
                    </div>
                </div>
            </div>
...
@push('javascript')
    ...
    <!--end::Form JS-->
    <!-- Custom JS -->
    @include('settings::category_types.creating_editing.scripts')
    <!--end::Custom JS-->
@endpush
