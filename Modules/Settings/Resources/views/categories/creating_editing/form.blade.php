...
@if (isset($category))
    @section('title', __('settings::dashboard.settings') . ' - ' . __('settings::dashboard.edit_category'))

    @section('subheader')
        @include('dashboard.layouts.partials.sub_header', [
            'module_name' => __('settings::dashboard.edit_category'),
            'short_description' => __('settings::dashboard.enter_category_details_and_submit'),
            'breadcrumbs' => [],
        ]);
    @endsection
@else
    @section('title', __('settings::dashboard.settings') . ' - ' . __('settings::dashboard.create_category'))
    @section('subheader')
        @include('dashboard.layouts.partials.sub_header', [
            'module_name' => __('settings::dashboard.new_category'),
            'short_description' => __('settings::dashboard.enter_category_details_and_submit'),
            'breadcrumbs' => [],
        ]);
    @endsection
    ...
    <div class="card card-custom">
        <div class="card-header">
            <div class="card-title">
                <h3 class="card-label">
                    {{ isset($category) ? __('settings::dashboard.edit_category') : __('settings::dashboard.new_category') }}
                    <i class="mr-2"></i>
                    <small class=""></small>
                </h3>
            </div>
            ...
            <div class="card-body">
                @if ($method == 'POST')
                    <div class="form-group row">
                        <label
                            class="col-1 col-form-label font-weight-bold text-center">{{ __('settings::dashboard.category_type') }}</label>
                        <div class="col-3">
                            <x-dashboard.form.inputs.select :id="'category_type'" :name="'category_type_id'" :options="$category_types"
                                :isMultiple="false" :defaultOptionName="__('settings::dashboard.select_category_type')" :selectedOption="old('category_type', $category->category_type_id ?? '')" />
                        </div>
                    </div>
                @endif

                <div id="kt_category_translation_repeater">
                    <div data-repeater-list="translations">
                        @forelse (old('translations', $category->translations ?? []) as $translation)
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-1">
                                <x-dashboard.form.inputs.success-switch class="mx-2" :id="'is_active_in_website'" :name="'is_active_in_website'"
                                    :isChecked="old('is_active_in_website', $category->is_active_in_website ?? '')" data-invisible-row-id="display_order_in_website_row" />
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label
                            ...
                            <div class="col-1">
                                <x-dashboard.form.inputs.success-switch class="mx-2" :id="'is_active_in_mobile'"
                                    :name="'is_active_in_mobile'" :isChecked="old('is_active_in_mobile', $category->is_active_in_mobile ?? '')"
                                    data-invisible-row-id="display_order_in_mobile_row" />
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label
                            ...
                            <div class="col-1">
                                <x-dashboard.form.inputs.success-switch class="mx-2" :id="'is_display_home_page_of_website'"
                                    :name="'is_display_home_page_of_website'" :isChecked="old(
                                        'is_display_home_page_of_website',
                                        $category->is_display_home_page_of_website ?? '',
                                    )"
                            ...
                            <div class="col-1">
                                <x-dashboard.form.inputs.success-switch class="mx-2" :id="'is_display_home_page_of_mobile'"
                                    :name="'is_display_home_page_of_mobile'" :isChecked="old(
                                        ...
                                        $category->is_display_home_page_of_mobile ?? '',
                                    )"
                            ...
                            <div class="col-1">
                                <x-dashboard.form.inputs.success-switch class="mx-2" :id="'is_display_in_fav_category_of_webite'"
                                    :name="'is_display_in_fav_category_of_webite'" :isChecked="old(
                                        ...
                                        $category->is_display_in_fav_category_of_webite ?? '',
                                    )" />
                            </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            <div class="col-1">
                                <x-dashboard.form.inputs.success-switch class="mx-2" :id="'is_display_in_fav_category_of_mobile'"
                                    :name="'is_display_in_fav_category_of_mobile'" :isChecked="old(
                                        ...
                                        $category->is_display_in_fav_category_of_mobile ?? '',
                                    )" />
                            </div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="row mb-3 @if (!old('is_active_in_website', $category->is_active_in_website ?? null)) invisible @endif"
                            ...
                            <div class="col-4">
                                <x-dashboard.form.inputs.number :id="'display_order_in_website'" :class="'form-control'" :name="'display_order_in_website'"
                                    :integerValidationMessage="__('settings::dashboard.display_order_must_be_integer')" :placeholder="__('settings::dashboard.display_order_in_website')" :value="old(
                                        ...
                                        $category->display_order_in_website ?? '',
                                    )" />
                            </div>
                        </div>
                        <div class="row mb-3 @if (!old('is_active_in_mobile', $category->is_active_in_mobile ?? null)) invisible @endif"
                            ...
                            <div class="col-4">
                                <x-dashboard.form.inputs.number :id="'display_order_in_mobile'" :class="'form-control'" :name="'display_order_in_mobile'"
                                    :integerValidationMessage="__('settings::dashboard.display_order_must_be_integer')" :placeholder="__('settings::dashboard.display_order_in_mobile')" :value="old('display_order', $category->display_order_in_mobile ?? '')" />
                            </div>
                        </div>
                        <div class="row mb-3 @if (!old('is_display_home_page_of_website', $category->is_display_home_page_of_website ?? null)) invisible @endif"
                            ...
                            <div class="col-4">
                                <x-dashboard.form.inputs.number :id="'display_order_in_home_page_of_website'" :class="'form-control'" :name="'display_order_in_home_page_of_website'"
                                    :integerValidationMessage="__('settings::dashboard.display_order_must_be_integer')" :placeholder="__('settings::dashboard.display_order_in_home_page_of_website')" :value="old(
                                        'display_order',
                                        $category->display_order_in_home_page_of_website ?? '',
                                    )" />
                            </div>
                        </div>
                        <div class="row mb-3 @if (!old('is_display_home_page_of_mobile', $category->is_display_home_page_of_mobile ?? null)) invisible @endif"
                            ...
                            <div class="col-4">
                                <x-dashboard.form.inputs.number :id="'display_order_in_home_page_of_mobile'" :class="'form-control'" :name="'display_order_in_home_page_of_mobile'"
                                    :integerValidationMessage="__('settings::dashboard.display_order_must_be_integer')" :placeholder="__('settings::dashboard.display_order_in_home_page_of_mobile')" :value="old(
                                        'display_order',
                                        $category->display_order_in_home_page_of_mobile ?? '',
                                    )" />
                            </div>
                        </div>
                    </div>
                ...
                <div class="form-group row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-8 col-xl-8">
                                <div class="image-input image-input-empty image-input-outline"
                                    style="background-image: url('{{ $category->image_url ?? global_asset('metronic/assets/media/users/blank.png') }}')"
                                    id="image">
                    ...
                    <div class="col-4">
                        <div class="row">
                            ...
                            <div class="col-lg-8 col-xl-8">
                                <div class="image-input image-input-empty image-input-outline"
                                    style="background-image: url('{{ $category->mobile_image_url ?? global_asset('metronic/assets/media/users/blank.png') }}')"
                                    id="mobile_image">
                                    <div class="image-input-wrapper"></div>
                                    <label
                                        class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow"
...
@push('javascript')
    ...
    <!--end::Form JS-->
    <!-- Custom JS -->
    @include('settings::categories.creating_editing.scripts')
    <!--end::Custom JS-->
@endpush
