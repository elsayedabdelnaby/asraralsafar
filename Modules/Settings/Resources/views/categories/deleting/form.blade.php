...
@section('content')
    <div class="modal-header">
        <h5 class="modal-title" id="deleteCategoryLabel">{{ __('settings::dashboard.delete_category') }}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
    </div>
    <form class="form parsley-form" action="{{ route('dashboard.settings.categories.destroy', ['id' => $item->id]) }}"
        method="POST">
        <div class="modal-body">
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    <input type="text" class="form-control" disabled value="{{ $item->category_name }}" />
                </div>
            </div>
            <div class="form-group row">
                <label for="recipient-name" class="col-4 col-form-label">{{ __('dashboard.replace_it_with') }}:</label>
                <div class="col-6">
                    <x-dashboard.form.inputs.select :id="'category'" :name="'replace_with'" :isRequired="true" :options="$other_items"
                        :requiredMessage="__('settings::dashboard.item_is_required')" :isMultiple="false" :defaultOptionName="__('settings::dashboard.select_category')" :selectedOption="old('replace_with')" />
                </div>
            </div>
        </div>
