...
@section('title', __('settings::dashboard.tags'));
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        <li class="breadcrumb-item text-muted">
                            <a href="{{ route('dashboard.settings.tags.index') }}"
                                class="text-muted">{{ __('settings::dashboard.tags') }}</a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
    ...
    <div class="card card-custom">
        <div class="card-header">
            <div class="card-title">
                ...
                <h3 class="card-label">{{ __('settings::dashboard.tags') }}</h3>
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{ route('dashboard.settings.tags.create') }}" class="btn btn-primary font-weight-bolder">
                    <i class="flaticon2-plus-1"></i>
                    {{ __('settings::dashboard.new_tag') }}</a>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
            <!--begin: Datatable-->
            <table class="table table-bordered table-hover table-checkable" id="kt_datatable_tags"
...
@push('javascript')
    ...
    @include('settings::tags.indexing.scripts');
@endpush
