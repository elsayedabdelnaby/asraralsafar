...
@section('content')
    <div class="modal-header">
        <h5 class="modal-title" id="deleteProfileLabel">{{ __('usersmanagement::dashboard.delete_profile') }}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
    </div>
    <form class="form parsley-form" action="{{ route('dashboard.users-management.profiles.destroy', ['id' => $item->id]) }}"
        method="POST">
        <div class="modal-body">
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    <x-dashboard.form.inputs.select :id="'profile'" :name="'replace_with'" :isRequired="true" :options="$other_items"
                        :requiredMessage="__('usersmanagement::dashboard.item_is_required')" :isMultiple="false" :defaultOptionName="__('usersmanagement::dashboard.select_profile')" :selectedOption="old('profile')" />
                </div>
            </div>
        </div>
