...
@section('title', __('website::dashboard.main_sliders'))
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        <li class="breadcrumb-item text-muted">
                            <a href="{{ route('dashboard.website.main-sliders.index') }}"
                                class="text-muted">{{ __('website::dashboard.main_sliders') }}</a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
    ...
    <div class="card card-custom">
        <div class="card-header">
            <div class="card-title">
                ...
                <h3 class="card-label">{{ __('website::dashboard.main_sliders') }}</h3>
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{ route('dashboard.website.main-sliders.create') }}" class="btn btn-primary font-weight-bolder">
                    <i class="flaticon2-plus-1"></i>
                    {{ __('website::dashboard.new_main_slider') }}</a>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
            <!--begin: Datatable-->
            <table class="table table-bordered table-hover table-checkable" id="kt_datatable_main_sliders"
                style="margin-top: 13px !important">
                <thead>
                    <tr>
                        <th>{{ __('website::dashboard.id') }}</th>
                        <th>{{ __('website::dashboard.image') }}</th>
...
@push('javascript')
    ...
    @include('website::main_sliders.indexing.scripts');
@endpush
