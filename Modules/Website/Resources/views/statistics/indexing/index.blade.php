...
@section('title', __('website::dashboard.statistics'));
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        <li class="breadcrumb-item text-muted">
                            <a href="{{ route('dashboard.website.statistics.index') }}"
                                class="text-muted">{{ __('website::dashboard.statistics') }}</a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
    ...
    <div class="card card-custom">
        <div class="card-header">
            <div class="card-title">
                ...
                <h3 class="card-label">{{ __('website::dashboard.statistics') }}</h3>
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{ route('dashboard.website.statistics.create') }}" class="btn btn-primary font-weight-bolder">
                    <i class="fas fa-user-shield"></i>
                    {{ __('website::dashboard.new_statistic') }}</a>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
            <!--begin: Datatable-->
            <table class="table table-bordered table-hover table-checkable" id="kt_datatable_statistics"
                style="margin-top: 13px !important">
                <thead>
                    <tr>
                        <th>{{ __('website::dashboard.id') }}</th>
                        <th>{{ __('website::dashboard.title') }}</th>
                        <th>{{ __('website::dashboard.number') }}</th>
...
@push('javascript')
    ...
    @include('website::statistics.indexing.scripts');
@endpush
